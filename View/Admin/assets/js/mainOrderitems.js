...
const dataOrders = async (id)=>{
    if(id) return await (new HTTPRequest("Orders").getOne(id));
    return null;
}
...
const data = async (location, count)=>{
   ...
   for (let [index, item] of res.data.entries()){
        const order =  await dataOrders(item.order_id)
        const product =  await dataProducts(item.product_id)
        const row = document.createElement("tr");
        row.innerHTML = `
            <td>${(location-1)*count+(index+1)}</td>
            <td>
                ${order?`<div class="accordion" id="accordionProducts">
                  <div class="accordion-item collapsed">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${item.item_id}">
                        ${"Số lượng: "+order.data.total_amount ?? ""}
                      </button>
                    </h2>
                    <div id="collapse${item.item_id}" class="accordion-collapse collapse">    
                      <div class="accordion-body">
                        <p>${order.data.status ?? ""} </p>
                        <p>${order.data.total_amount ?? ""} </p>
                      </div>
                    </div>
                  </div>
                </div>`: ""}
            ...
            <td>
                ${product?`<div class="accordion" id="accordionProducts">
                  <div class="accordion-item collapsed">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse1${item.item_id}">
                        ${product.data.name ?? ""}
                      </button>
                    </h2>
                    <div id="collapse1${item.item_id}" class="accordion-collapse collapse">    
                      <div class="accordion-body">
            ...
            <td>
                <div class="btn-group border border-none gap-2" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-warning rounded" onclick="editRow(${item.item_id}, ${location})">Sửa</button>
                    <button type="button" class="btn btn-outline-danger rounded" onclick="deleteRow(${item.item_id}, ${location})">Xóa</button>
                </div>
            </td>
        `
    tbody.append(row);
...
async function editRow(id, currentPage){
    ...
    const orders = new HTTPRequest("Orders").getAll().then(value => {
        ...
        const ordersID = modal.querySelector(`#order_id>option[value='${dataOld.data.order_id ?? ''}']`);
        if(ordersID!==null) ordersID.setAttribute("selected", "");
    });
