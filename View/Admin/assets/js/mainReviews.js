...
const data = async (location, count) => {
  ...
  for (let [index, item] of response.data.entries()) {
    ...
    row.innerHTML = `
            ...
            <td>
              <button type="button" class="btn btn-warning" onclick="editRow(${item.review_id}, ${location})">Sửa</button>
              <button type="button" class="btn btn-outline-danger mt-xxl-0" onclick="deleteRow(${item.review_id}, ${location})">Xóa</button>
            </td>
        `;
    tbody.append(row)
  };
