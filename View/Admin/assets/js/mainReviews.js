...
const data = async (location, count) => {
  ...
  for (let [index, item] of response.data.entries()) {
    ...
    const user = await getDataUser(item.user_id ?? false);
    row.innerHTML = `
            <td>${index + 1}</td>
            <td>
              ${product ? `<div class="accordion" id="accordionProducts">
                  <div class="accordion-item collapsed">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${item.review_id}">
                        ${product.data.name ?? ""}
                      </button>
                    </h2>
                    <div id="collapse${item.review_id}" class="accordion-collapse collapse">
            ...
            <td>
                ${user ? `<div class="accordion" id="accordionProducts">
                  <div class="accordion-item collapsed">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse1${item.review_id}">
                        ${user.data.name ?? ""}
                      </button>
                    </h2>
                    <div id="collapse1${item.review_id}" class="accordion-collapse collapse">
                      <div class="accordion-body">
                        <p>${user.data.email ?? ""}</p>
                        <p>${user.data.phone ?? ""}</p>
            ...
            <td>${item.created_at ?? ""}</td>
             <div class="btn-group border border-none gap-2" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-warning rounded" onclick="editRow(${item.review_id}, ${location})">Sửa</button>
                    <button type="button" class="btn btn-outline-danger rounded" onclick="deleteRow(${item.review_id}, ${location})">Xóa</button>
            </div>
        `;
    tbody.append(row)
  };
...
async function editRow(id, currentPage) {
  ...
  const users = new HTTPRequest("Users").getAll().then(value => {
    ...
    const userId = modal.querySelector(`#user_id>option[value='${dataOld.data.user_id ?? ''}']`);
    if (userId !== null) userId.setAttribute("selected", "");
  });
  modal.querySelector("#comment").value = dataOld.data.comment ?? "";
  modal.querySelector("#product_id").value = dataOld.data.product_id ?? "";
  modal.querySelector("#user_id").value = dataOld.data.user_id ?? "";
  ...
  validate.checkFormAndDisplay(checForms);
  const handleSubmit = async e => {
    e.preventDefault();
    if (validate.checkForm(checForms, true)) {
