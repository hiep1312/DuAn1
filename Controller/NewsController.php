<?php
    ...
    class NewsController
    {
        ...
        public function create($dataRequest, $file){
            ...
            if($file["image_url"]['error']===UPLOAD_ERR_OK || $file["image_url"]['error'][0]===UPLOAD_ERR_OK){
                if(gettype($file["image_url"]["name"])==="array"){
                    $imagePath = BASE_IMAGE . "News/" . time() . $file["image_url"]["name"][0];
                }else{
                    $imagePath = BASE_IMAGE . "News/" . time() . $file["image_url"]["name"];
                }
                move_uploaded_file(gettype($file["image_url"]["tmp_name"])==="array"?$file["image_url"]["tmp_name"][0]:$file["image_url"]["tmp_name"], $imagePath);
